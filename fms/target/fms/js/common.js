...
var urlhost = "http://www.fmse.cn/fms/";
...
function wecharauto2hosturl(url) {
	...
	+ "&response_type=code&scope=snsapi_base&state=fms#wechat_redirect";
}
...
function browse_record(code) {
	...
	url = url.substring(0, url.indexOf("code") - 1);// 截取code以及之后的数据
	ajaxUtilAsync({
		"code" : code,
		"url" : url
...
function share_WeChat(title, imgUrl, desc, link) {
	...
	if (isEmpty(imgUrl)) {
		imgUrl = urlhost + "/images/fms-logo-04large.jpg";
	}
